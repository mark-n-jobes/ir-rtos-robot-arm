...
void draw() {
    ...
    if(WiiRemoteEnabled == 1) WiiRemote.UpdatePoints(ScreenWidth/3.,ScreenHeight*2./3.);
    //-----------Begin Draw------------//
    // WiiRemote
...
class ArmSegmentChain {
    ...
    //------------------------------------------------------------------//
    void AlignHandToPoint(Point In, Point Origin) {
        ...
        float Side = (AdjustedIn.X>this.Points[0].X)? -1. : 1.;
        ...
        if(Dist_P2P0 < (this.ArmLengths[1]+this.ArmLengths[0])) { // Then easy solution exists
            ...
            Point NewP1 = new Point(this.ArmLengths[1], 0.0, this.Points[1].Z);
